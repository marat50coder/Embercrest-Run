...
/// Full-screen WebView shell for the gray path.
class EmberPortal extends StatefulWidget {
  const EmberPortal({
    super.key,
    required this.url,
    required this.vault,
  ...
  @override
  State<EmberPortal> createState() => _EmberPortalState();
}

class _EmberPortalState extends State<EmberPortal> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    _networkSubscription = widget.watch.changes.listen((states) {
  ...
  Future<void> _settleColdViewport() async {
    _enterImmersive();
    // Settle in the phone's ACTUAL orientation before mounting — no rotation
    // nudge (that made cold-start links open sideways then flip). Residual
    // stretch is fixed by the post-load resize + single reload below.
  ...
  Future<void> _showOfflineAfterProbe() async {
    ...
    bool online = true;
    try {
      online = await widget.watch.canReachNetwork();
    } catch (_) {
      online = false;
    }
    if (online) return;
...
(() => {
  const root = window;
  if (root.__embInsetGuard) return;
  root.__embInsetGuard = true;
  const marker = 'emb-inset-sheet';
  const rules = [
    ':root{',
...
''');
  ...
  void _installZoomLock() {
    _controller.runJavaScript(r'''
(() => {
  if (window.__embZoomHold) return;
  window.__embZoomHold = true;
  const lockViewport = () => {
    const host = document.head || document.documentElement;
    if (!host) return;
...
''');
  ...
  void _installTapPolish() {
    _controller.runJavaScript(r'''
(() => {
  if (window.__embTapClean) return;
  window.__embTapClean = true;
  const style = document.createElement('style');
  style.id = 'emb-tap-clean';
  style.textContent =
...
''');
  ...
  void _installKeyboardLift() {
    _controller.runJavaScript(r'''
(() => {
  if (window.__embInputRaise) return;
  window.__embInputRaise = true;
...
''');
  ...
  void _installFocusScaleGuard() {
    ...
    _controller.runJavaScript(r'''
(() => {
  if (window.__embFocusScale) return;
  window.__embFocusScale = true;
  const style = document.createElement('style');
  style.textContent =
...
''');
  ...
  void _installInlinePlayback() {
    _controller.runJavaScript(r'''
(() => {
  if (window.__embInlinePlay) return;
  window.__embInlinePlay = true;
  const awaken = (video) => {
